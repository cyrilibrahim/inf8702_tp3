...
const CCouleur CScene::ObtenirCouleurSurIntersection( const CRayon& Rayon, const CIntersection& Intersection ) const
{
	...
	if( RefractedRayonEnergy > m_EnergieMinRayon && Rayon.ObtenirNbRebonds() < m_NbRebondsMax )
	{
		...
		RefractedRayon.AjusterDirection(CVecteur3::Refract(Rayon.ObtenirDirection(), Intersection.ObtenirNormale(), IndiceRefractionRatio));
...
const CCouleur CScene::ObtenirFiltreDeSurface( CRayon& LumiereRayon ) const
{
	...
	CIntersection intersect;
	for (SurfaceIterator aSurface = m_Surfaces.begin(); aSurface != m_Surfaces.end(); aSurface++)
	{
		intersect = (*aSurface)->Intersection(LumiereRayon);
		// S'il y a une intersection appliquer la translucidité de la surface
		// intersectée sur le filtre
		if (intersect.ObtenirDistance() > EPSILON) {
			ISurface *surf = intersect.ObtenirSurface();
			Filter *= surf->ObtenirCouleur() * surf->ObtenirCoeffRefraction();
		}
	}
